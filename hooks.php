 ...
 * 
 * @package KsfFA_ksf_FA_TravelExpense
 * @version 2.4.3-0
 */

define('SS_ksf_FA_TravelExpense', 141 << 8);

class hooks_ksf_FA_TravelExpense extends hooks {
    var $module_name = 'ksf_FA_TravelExpense';
    var $version = '2.4.3-0';
    ...
    function install_access() {
        $security_sections[SS_ksf_FA_TravelExpense] = _("Travel Expense");
        $security_areas['SA_ksf_FA_TravelExpenseVIEW'] = array(
            SS_ksf_FA_TravelExpense | 1, 
            _("View ")
        );
        $security_areas['SA_ksf_FA_TravelExpenseMANAGE'] = array(
            SS_ksf_FA_TravelExpense | 2, 
            _("Manage ")
        );
    ...
    private function ensure_composer_dependencies() {
        ...
        chdir($prev_dir);
        if ($return_code !== 0) {
            error_log('ksf_FA_TravelExpense: composer install failed: ' . implode("\n", $output));
        }
    }
}
